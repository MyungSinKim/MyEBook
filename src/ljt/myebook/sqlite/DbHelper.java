...
public class DbHelper extends SQLiteOpenHelper {
	...
	public final static String FIELD_SAVETIME = "save_time";
	...
	public long insert(String filename, String bookmark,String save_time) {
		SQLiteDatabase db = this.getWritableDatabase();
		ContentValues cv = new ContentValues();
		cv.put(FIELD_FILENAME, filename);
		cv.put(FIELD_BOOKMARK, bookmark);
		cv.put(FIELD_SAVETIME, save_time);
		long row = db.insert(TABLE_NAME, null, cv);
		return row;
	}
	...
	public void update(int id, String filename, String bookmark,String save_time) {
		SQLiteDatabase db = this.getWritableDatabase();
		String where = FIELD_ID + "=?";
		String[] whereValue = { Integer.toString(id) };
		ContentValues cv = new ContentValues();
